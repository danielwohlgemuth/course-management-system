...
function sf(command) {
  ...
  const raw = execSync(`sf ${command} --json ${targetFlag()}`, {
    encoding: "utf8",
    env
  });
...
/** Runs a SOQL query against the default org and returns the records array */
export function query(soql) {
  return sf(`data query --query "${soql.replace(/"/g, '\\"')}"`).records;
}
