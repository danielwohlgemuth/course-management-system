...
export default class CourseCalendar extends NavigationMixin(LightningElement) {
  ...
  wiredPicklistValues({ data, error }) {
    if (data) {
      ...
      logError(
        "courseCalendar",
        "wiredPicklistValues: " + JSON.stringify(error),
        ""
      );
    }
  }
  ...
  wiredSemesterValues({ data, error }) {
    if (data) {
      ...
      logError(
        "courseCalendar",
        "wiredSemesterValues: " + JSON.stringify(error),
        ""
      );
    }
  }
  ...
  handleGroupClick(event) {
    ...
    const left =
      event.clientX + 8 + popoverWidth > window.innerWidth
        ? event.clientX - 8 - popoverWidth
        : event.clientX + 8;
    const top =
      event.clientY + popoverHeight > window.innerHeight
  ...
  _rebuildIfReady() {
    if (this._slots && this._days && this._config) {
      this.calendarDays = this._buildCalendar(
        this._applyFilters(this._slots),
        this._days
      );
    }
  }
  ...
  _buildCalendar(slots, days) {
    ...
    return days.map((d) => {
      const daySlots = renderablesByDay.get(d.value);
      return {
        key: d.value,
        label: d.label,
        slots: daySlots,
        hasOverlap: daySlots.some((s) => s.isGroup)
      };
    });
  }
  ...
  _resolveOverlaps(daySlots, gridStartHour, totalHours) {
    ...
    for (let i = 0; i < daySlots.length; i++) {
      for (let j = i + 1; j < daySlots.length; j++) {
        if (
          daySlots[i].startMs < daySlots[j].endMs &&
          daySlots[i].endMs > daySlots[j].startMs
        ) {
          union(i, j);
        }
      }
    ...
    for (const indices of groups.values()) {
      if (indices.length === 1) {
        ...
        const height =
          hoursToPct(msToHours(s.endMs), gridStartHour, totalHours) - top;
        renderables.push({
          key: s.key,
          isCard: true,
        ...
        const groupStartMs = Math.min(
          ...indices.map((i) => daySlots[i].startMs)
        );
        const groupEndMs = Math.max(...indices.map((i) => daySlots[i].endMs));
        const groupDurationMs = groupEndMs - groupStartMs;
        const top = hoursToPct(
          msToHours(groupStartMs),
          gridStartHour,
          totalHours
        );
        const height =
          hoursToPct(msToHours(groupEndMs), gridStartHour, totalHours) - top;

        const n = indices.length;
        const slotsSorted = indices
          .map((i) => daySlots[i])
